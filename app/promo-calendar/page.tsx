...
type PromoType = "MCB" | "Scan" | "OI" | "Passport"
...
interface PromoEvent {
  ...
  name: string
  type: PromoType
  startPeriod: number // 0-indexed into periods
  spanPeriods: number
}
...
const retailerData: RetailerRow[] = [
  {
    ...
    promos: [
      ...
      { id: "wf-4", name: "15% OI", type: "OI", startPeriod: 3, spanPeriods: 1 },
    ],
  },
  {
    name: "BRISTOL FARMS",
...
export default function PromoCalendarPage() {
  ...
  const addFundingLine = () => {
    setFundingLines([...fundingLines, { type: "Scan", amount: "", unit: "Per Case" }])
  }

  const removeFundingLine = (index: number) => {
    setFundingLines(fundingLines.filter((_, i) => i !== index))
  }
              ...
              <div className="p-6 lg:p-8 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      Plan promotions by retailer. Compare scenarios before committing.
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      size="sm"
                      className="bg-[#6C12ED] hover:bg-[#5A0ECB] text-white"
                ...
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                  ...
                  <SummaryCard
                    ...
                    value="$83.0K"
                    sub="Based on promo agreements"
                    icon={<ClipboardList className="h-4 w-4 text-gray-500" />}
                  />
                  <SummaryCard
                    label="ACTUAL DEDUCTIONS"
                    value="$57.9K"
                    sub="Deductions received to date"
                    icon={<DollarSign className="h-4 w-4 text-gray-500" />}
                  />
                  <SummaryCard
                    label="NET VARIANCE"
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3">
                    <Select value={quarterFilter} onValueChange={setQuarterFilter}>
                      <SelectTrigger className="w-[130px] bg-gray-900 border-gray-700 text-gray-200 text-sm">
                        <SelectValue />
                      </SelectTrigger>
                    ...
                    <Select value={retailerFilter} onValueChange={setRetailerFilter}>
                      <SelectTrigger className="w-[150px] bg-gray-900 border-gray-700 text-gray-200 text-sm">
                ...
                <div className="flex items-center gap-6 text-xs text-gray-400">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-sm bg-[#6C12ED]" />
                    <span>MCB</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-sm bg-[#0EA5E9]" />
                    <span>Scan / Scan-Back</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-sm bg-[#F59E0B]" />
                    <span>OI (Off-Invoice)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-sm bg-[#92700C]" />
                    <span>Passport / Program</span>
                  </div>
                </div>
                ...
                <Card className="bg-gray-900 border-gray-800 overflow-hidden">
                  <CardContent className="p-0">
                    {/* Period Headers */}
                    <div className="grid grid-cols-[200px_1fr] border-b border-gray-800">
                    ...
                    {retailerData.map((retailer, rIdx) => {
                      // Group promos into visual rows to handle stacking
                      ...
                      retailer.promos.forEach((promo) => {
                        ...
                        for (const row of promoRows) {
                          const overlaps = row.some(
                            (existing) =>
                              promo.startPeriod < existing.startPeriod + existing.spanPeriods &&
                              promo.startPeriod + promo.spanPeriods > existing.startPeriod
                          )
                          if (!overlaps) {
                            row.push(promo)
                            placed = true
                        ...
                        if (!placed) {
                          promoRows.push([promo])
                        }
                      })

                      const rowHeight = promoRows.length > 0 ? Math.max(promoRows.length * 36 + 16, 52) : 52

                      return (
                        <div
                          key={retailer.name}
                          className={`grid grid-cols-[200px_1fr] ${
                            rIdx < retailerData.length - 1 ? "border-b border-gray-800" : ""
                          }`}
                          style={{ minHeight: rowHeight }}
                        >
                          {/* Retailer Name */}
                          <div className="flex items-center px-4 border-r border-gray-800 bg-gray-900/50">
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            <DialogContent className="bg-gray-900 border-gray-800 text-gray-50 max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6 py-2">
                    ...
                    {templates.map((t) => (
                      <div key={t.id} className="flex items-center gap-3">
                        <RadioGroupItem
                          value={t.id}
                          id={t.id}
                          className="border-gray-600 text-[#6C12ED] data-[state=checked]:bg-[#6C12ED] data-[state=checked]:border-[#6C12ED]"
                        />
                        <Label htmlFor={t.id} className="text-sm text-gray-300 cursor-pointer font-normal">
                          {t.label}
                        </Label>
                      </div>
                    ))}
                ...
                <section>
                  <Label className="text-xs text-gray-400 mb-1.5 block">
                    Offer Description <span className="text-red-400">*</span>
                  </Label>
                  <Input
                    value={modalDescription}
                    onChange={(e) => setModalDescription(e.target.value)}
                    placeholder="e.g., 2/$5 Spring Promo"
                ...
                <section>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      onClick={addFundingLine}
                      className="text-xs font-medium text-[#6C12ED] hover:text-[#8B3FF5] transition-colors"
                    >
                      + Add Line
                    </button>
                  </div>
                    ...
                    {fundingLines.map((line, index) => (
                      <div key={index} className="flex items-center gap-3">
                        <Select
                          value={line.type}
                          onValueChange={(val) => {
                            const updated = [...fundingLines]
                            updated[index].type = val
                            setFundingLines(updated)
                          }}
                        >
                          <SelectTrigger className="w-[120px] bg-gray-800 border-gray-700 text-gray-200 text-sm">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent className="bg-gray-800 border-gray-700">
                            <SelectItem value="Scan">Scan</SelectItem>
                        ...
                        <Input
                          ...
                          placeholder="0.00"
                          className="w-[100px] bg-gray-800 border-gray-700 text-gray-200 text-sm"
                        />
                        <Select
                          value={line.unit}
                          onValueChange={(val) => {
                        ...
                        >
                          <SelectTrigger className="w-[140px] bg-gray-800 border-gray-700 text-gray-200 text-sm">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent className="bg-gray-800 border-gray-700">
                        ...
                        {fundingLines.length > 1 && (
                          <button
                            onClick={() => removeFundingLine(index)}
                ...
                <section>
                  ...
                  <Textarea
                    ...
                    placeholder="e.g., Part of Passport to Savings program..."
                    className="bg-gray-800 border-gray-700 text-gray-200 placeholder:text-gray-600 min-h-[80px]"
                  />
                </section>
              </div>
